...
def to_string(obj) -> str:
    if isinstance(obj, bool):
        ...
        return f"五档行情{{合约:{obj.symbol}.{obj.exchange.value} 时间:{to_string(obj.datetime)} 最新价: {obj.last_price} 最新交易量:{obj.last_volume} " \
        ...
        return f"记录行情{{间隔:{obj.interval.value} 合约:{obj.symbol}.{obj.exchange.value} 时间:{to_string(obj.datetime)} " \
        ...
        return f"合约:{obj.symbol}.{obj.exchange.value} 名称:{obj.name} 类型:{obj.product.value} 合约乘数:{obj.size} 最小变动价位:{obj.pricetick} " \
        ...
        return f"一档行情{{合约:{obj.symbol}.{obj.exchange.value} 行情号:{obj.quoteid} 时间:{to_string(obj.datetime)} 状态:{obj.status.value} 引用:{obj.reference} " \
